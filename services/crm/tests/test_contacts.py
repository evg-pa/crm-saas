...
@pytest.mark.asyncio
async def test_create_contact(client, test_org_id):
...
@pytest.mark.asyncio
async def test_list_contacts(client, test_org_id):
    ...
    resp = await client.get(CONTACTS_URL)
...
@pytest.mark.asyncio
async def test_list_contacts_search(client, test_org_id):
    ...
    resp = await client.get(CONTACTS_URL, params={"q": "Zelda"})
    assert resp.status_code == 200, resp.text
    data = resp.json()
    assert data["total"] == 1
...
@pytest.mark.asyncio
async def test_get_contact(client, test_org_id):
    ...
    resp = await client.get(f"{CONTACTS_URL}/{contact_id}")
    assert resp.status_code == 200, resp.text
...
@pytest.mark.asyncio
async def test_update_contact(client, test_org_id):
...
@pytest.mark.asyncio
async def test_delete_contact(client, test_org_id):
    ...
    resp = await client.delete(f"{CONTACTS_URL}/{contact_id}")
    assert resp.status_code == 204, resp.text

    # Verify it's gone
    get_resp = await client.get(f"{CONTACTS_URL}/{contact_id}")
...
@pytest.mark.asyncio
async def test_create_contact_empty_name(client, test_org_id):
...
@pytest.mark.asyncio
async def test_update_contact_empty_first_name(client, test_org_id):
...
@pytest.mark.asyncio
async def test_get_contact_not_found(client, test_org_id):
    """GET /contacts/{id} — non-existent contact."""
    fake_id = str(uuid.uuid4())
    resp = await client.get(f"{CONTACTS_URL}/{fake_id}")
    assert resp.status_code == 404, resp.text
    assert resp.json()["detail"] == "Contact not found"


@pytest.mark.asyncio
async def test_update_contact_not_found(client, test_org_id):
...
@pytest.mark.asyncio
async def test_delete_contact_not_found(client, test_org_id):
    """DELETE /contacts/{id} — non-existent contact."""
    fake_id = str(uuid.uuid4())
    resp = await client.delete(f"{CONTACTS_URL}/{fake_id}")
    assert resp.status_code == 404, resp.text
