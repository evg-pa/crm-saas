...
async def create_deal(
    ...
    deal = await _repo(db).create(**data)
...
async def list_deals(
    ...
    """List deals for the authenticated user's organization with optional search and filters."""
...
async def get_deal(
    ...
    """Get a deal by ID."""
    deal = await _repo(db).get_by_id(deal_id, current_user.organization_id)
    if deal is None:
